...
- forth Standard-II pictured output (§V34): `<# # #s #> hold sign` + `.r u.r`. ANS-exact on `ud` (rides `um/mod`). digit chars → fixed HOLD buffer below harness (§V35); running value = `ud` on data stack.
...
V32: (Standard-II) double combine is SIGN-sensitive — pin per word. `hi<<32 | (lo>>>0)` = the UNSIGNED 64-bit pattern; `um* um/mod ud.` want exactly that. SIGNED words (`m* sm/rem fm/mod d. d.r dnegate dabs d+ d-`) ! reinterpret via `BigInt.asIntN(64, pattern)` before compute, re-encode via `asUintN(64,·)` after. miss it → small positives look right, negatives/large break (== V29 `u<` failure shape). test: `-1 s>d d.`→`-1` but `-1 s>d ud.`→`18446744073709551615`.
V33: (Standard-II) `fm/mod` ≠ `sm/rem` = different algorithms, ⊥ sign tweaks. BigInt `/`,`%` truncate toward zero = SYMMETRIC = `sm/rem ( d n -- rem quot )`. `fm/mod ( d n -- rem quot )` = FLOORED: if rem≠0 & sign(rem)≠sign(n) → quot-=1, rem+=n. naive BigInt makes both identical ∴ `fm/mod` wrong for negatives. test: `-7 2 fm/mod`→`1 -4` (floored); `-7 2 sm/rem`→`-1 -3` (symmetric). if equal, one is broken. div-by-0 both → `THROW -10`.
...
V35: (Standard-II) HOLD = fixed buffer, `HOLD_SIZE` bytes at `[harness-HOLD_SIZE, harness)`; `Memory.hold` = region top (analogous to `Memory.harness`). fills downward (holdPtr starts at `hold`, decrements). HOLD_SIZE ≥ 66 (64-bit ud in base 2 = 64 digit bytes + sign) → cell-align 68. §V7 guard protects it. `<#` re-inits holdPtr ∴ well-formed `<# … #>` safe; abort()/reset() clear holdPtr for §B2-consistency (low-pri, `<#` re-inits anyway).
...
T32|.|Standard-II: trivials `bl c, count within pick 2@ 2! chars char+ .( abort"` (within=unsigned, abort"=inline-str+THROW -2, 2@/2! ANS order §V31, pick past depth→-4)|I.forth,V23,V31
